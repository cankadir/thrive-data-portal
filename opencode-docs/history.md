...
- **Verified**: Svelte autofixer clean on both components; `npm run build` clean; temp dev server on `:5199` returned 41 records / 82 edit links (label + Edit per row) with correct `globalId`s, and 404 for an unknown slug. (Row-link rewrite verified with autofixer + prettier only — build skipped to avoid clobbering a dev server the user was starting.)
